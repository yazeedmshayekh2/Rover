...
def extract_text(image, prompt=None):
    ...
    if not prompt or prompt.strip() == "":
        ...
        analyze_messages = [
            {
                ...
                "content": [
                    ...
                    {"type": "text", "text": "What type of document is this? Is it an ID card, receipt, business card, form, handwritten note, or something else? Just answer with the document type."}
                ]
            }
        ]
        ...
        try:
            ...
            if "id" in analyze_output or "card" in analyze_output and ("identity" in analyze_output or "national" in analyze_output):
                prompt = """Extract all text from this ID card systematically. Format as follows:
1. Document title (e.g. National ID, Identity Card, etc.)
2. ID/Card number - include the complete number
3. Name - full name as shown
4. Date of birth - in the exact format shown
5. Place of birth (if available)
6. Gender/Sex
7. Nationality (if available)
8. Expiry date (if available)
9. Issue date (if available)
10. Other fields with their labels and values
                
For any Arabic or non-English text, include both the original text and the transliteration or translation if you can identify it.
Present the information in a clean numbered list with clear labels."""
